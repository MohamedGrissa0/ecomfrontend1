    ...
    <div>
      <div className="mx-auto container h-full">
        <div className="grid relative grid-cols-10 gap-5 py-10 px-4 md:px-0 lg:px-16 h-full">
          ...
          {/* Sidebar */}
          <div
            className={`absolute md:relative top-0 left-0 h-full p-8 bg-white md:col-span-3 lg:col-span-2 transform ${
              isSidebarOpen ? "animate__animated animate__slideInLeft" : ""
                ...
                {products.map((p, idx) => (
                  <label key={idx} className="inline-flex items-center">
                    <input
                      ...
                      checked={selectedBrands.includes(p.brand)}
                      onChange={() => handleBrandChange(p.brand)}
                    />
                    <span className="ml-2">{p.brand}</span>
                  </label>
                ))}
              </div>
            </div>
